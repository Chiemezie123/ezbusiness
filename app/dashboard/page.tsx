...
export default function Page() {
  ...
  return (
    <main className={`${isDarkMode ? 'bg-[#12121E]' : 'bg-[#F3F3F3]'} h-full w-full xl:px-[40.43px] 2xl:px-[79.43px] xl:pt-[20px] 2xl:pt-[24.25px] flex xl:gap-[15px] 2xl:gap-[20.42px]`}>
      <section className="flex flex-col xl:gap-[20.42px] 2xl:gap-[20.42px] xl:w-[500px] 2xl:w-[521.255px]">
        ...
        <div className="w-full px-[23px] py-[22px] h-full">
          <TitleCard header="Overall User Activity" date={"2023"}/>
          <LineChart/>
          </div>
        </div>
      </section>
      <section className="flex-1 flex flex-col gap-[20.42px]">
        ...
        <div className="w-full flex gap-[14.8px] ">
          {moreDetails.map((items,index)=>(
            ...
            <ThirdDisplayCard key={index} header={items.header} icon1={items.icon1} icon2={items.icon2} invoice={items.invoice} content={items.content}/>
          ))}
        </div>
        <div className={`w-full h-[267.335px] ${isDarkMode ? 'bg-[#383848]' : 'bg-[#fff]'} rounded-[10.669px]`}>
        <div className="w-full px-[23px] py-[22px]">
